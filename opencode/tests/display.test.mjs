...
test("real opencode events render as a guided transcript with a closing digest", async () => {
  ...
  assert.ok(digest.includes("Resumen"))
  assert.ok(digest.some((line) => /^ 1 ✔ Leyó note\.txt$/.test(line)), digest.join("\n"))
  assert.ok(digest.some((line) => /^ 2 ✔ Ejecutó echo done  salida 0$/.test(line)))
