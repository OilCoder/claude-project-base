...
# Executing a gate (bash adw/gates/phase-N.sh) is allowed and encouraged.
set -u
  ...
  Bash)
    ...
    if printf '%s' "$cmd" | grep -q 'adw/gates'; then
      ...
      if printf '%s' "$cmd" | grep -qE '(>|>>)[[:space:]]*[^[:space:]]*adw/gates|(rm|mv|cp|tee|chmod|truncate)[[:space:]][^|;]*adw/gates|sed[[:space:]]+-i[^|;]*adw/gates'; then
        echo "BLOCKED: this command would modify adw/gates/. The builder cannot touch gates — run them (bash adw/gates/phase-N.sh), never rewrite them." >&2
        exit 2
      fi
    fi
    ;;
